...
public abstract class BaseSeries<E extends DataPointInterface> implements Series<E> {
    ...
    public void shiftSeriesXValues(double startIndex, double endIndex, double shiftAmount) {
        for (double i = startIndex; i < endIndex; i++) {
            DataPoint currPoint = (DataPoint) mData.get((int) i);
            currPoint.setX(currPoint.getX() + shiftAmount);
        }
    }
